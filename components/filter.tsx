...
const FilterButton = ({ children, active }: any) => {
    return <button className={cn('p-2 px-3 rounded-xl text-muted-foreground', active && 'bg-sky-500/40 text-neutral-800')}>{children}</button>;
};
    ...
    return (
        <div className="bg-[#F4F6F9] rounded-xl p-6 space-y-4">
            ...
            <div className="flex drop-shadow-2xl items-end w-full">
                <FilterInput label="City or hotel" placeholder="Search" className="rounded-l-xl rounded-r-none" />
                <FilterInput label="Street or Address" placeholder="Search" className="rounded-l-none rounded-r-none" />
                <FilterInput label="Story or House" placeholder="Search" className="rounded-l-none rounded-r-none" />
